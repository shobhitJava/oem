...
fmt.Printf("creating record for potable")
	switch function {
...
func (t *OEM) Query(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	fmt.Printf("getting po table record")
fmt.Printf("key for row is "+ args[0])
	switch function {
	...
	if len(args) < 1 {
		...
		v := t.convert(row)
		//fmt.Print("the value could be "+v.dealerName)
